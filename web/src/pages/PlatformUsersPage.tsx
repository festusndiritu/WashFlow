...
const ROLE_BADGE: Record<string, string> = {
  ...
  worker: 'bg-stone-100 text-secondary',
}
  ...
  return (
    <AppShell orgName="Platform Console" orgRole="Platform Owner" sidebarNav={<PlatformNav />}>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-base font-semibold text-primary leading-none mb-0.5">Platform Users</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-theme bg-subtle">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider hidden sm:table-cell">Organization(s)</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider hidden md:table-cell">Role(s)</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider hidden lg:table-cell">Joined</th>
                ...
                <tr key={u.id} className="hover:bg-subtle transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-stone-200 flex items-center justify-center text-secondary font-bold text-sm shrink-0 select-none">
                    ...
                    {u.tenants.length === 0
                      ...
                      : <div className="flex flex-wrap gap-1">
                          {u.tenants.slice(0, 2).map(t => (
                            <span key={t} className="text-xs bg-stone-100 text-secondary px-1.5 py-0.5 rounded">{t}</span>
                          ))}
                          {u.tenants.length > 2 && <span className="text-xs text-tertiary">+{u.tenants.length - 2}</span>}
                        </div>}
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <div className="flex flex-wrap gap-1">
                      {u.roles.map(r => (
                        <span key={r} className={`text-[11px] font-semibold px-2 py-0.5 rounded-full capitalize ${ROLE_BADGE[r] ?? 'bg-stone-100 text-secondary'}`}>{r}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-tertiary hidden lg:table-cell">{formatDate(u.created_at)}</td>
                  <td className="px-4 py-3">
