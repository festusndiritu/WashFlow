...
const CATEGORY_COLOR: Record<string, string> = {
  ...
  general: 'bg-stone-100 text-secondary border-stone-200 hover:bg-stone-200',
...
export function OrdersPage() {
  ...
  const headerSlot = (
    <div className="flex items-center justify-between gap-4 flex-wrap">
      <div>
        <h1 className="text-base font-semibold text-primary leading-none mb-0.5">Orders</h1>
    ...
    <AppShell orgName={tenant?.name ?? 'Organization'} orgRole={user?.role ?? 'worker'} sidebarNav={sidebarNav} headerSlot={headerSlot}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="card w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <div><h3 className="text-base font-semibold text-primary">Record payment</h3><p className="text-xs text-tertiary mt-0.5">{customerMap[payingOrder.customer_id]?.name ?? 'Order'}</p></div>
              <button onClick={() => setPayingOrder(null)} className="p-1.5 text-tertiary hover:text-secondary rounded-lg hover:bg-stone-100"><X className="w-4 h-4" /></button>
            </div>
            <form onSubmit={submitPayment} className="space-y-4">
              <div className="flex gap-2">
                {(['cash', 'mpesa'] as const).map((m) => (
                  <button key={m} type="button" onClick={() => setPayMethod(m)}
                    className={`flex-1 py-2 rounded-lg text-sm font-semibold border transition-colors ${payMethod === m ? 'bg-orange-500 text-white border-orange-500' : 'bg-surface text-secondary border-theme hover:border-stone-300'}`}>
                    {m === 'cash' ? '\u{1F4B5} Cash' : '\u{1F4F1} M-Pesa'}
                  </button>
                ))}
              </div>
              <div>
                <label className="block text-xs font-medium text-secondary mb-1.5">Amount (KES)</label>
              ...
              {payMethod === 'mpesa' && (
                <div>
                  <label className="block text-xs font-medium text-secondary mb-1.5">M-Pesa reference (optional)</label>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-[480px] shadow-2xl flex flex-col h-full" style={{ background: "var(--bg-surface)" }}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-stone-800 bg-stone-950 shrink-0">
              <div><h2 className="text-base font-semibold text-white">New order</h2><p className="text-xs text-tertiary mt-0.5">{shopName}</p></div>
              <button onClick={closePanel} className="p-1.5 text-tertiary hover:text-white rounded-lg hover:bg-stone-800 transition-colors"><X className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              <form id="pos-form" onSubmit={onSubmit}>
                ...
                <div className="px-5 py-4 border-b border-theme">
                  <p className="text-[11px] font-semibold text-tertiary uppercase tracking-wider mb-3">Customer</p>
                  <button type="button" onClick={handleWalkIn} disabled={walkInCreating}
                    ...
                    <div className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center shrink-0"><User className="w-4 h-4 text-secondary" /></div>
                    <div className="text-left">
                      <p className="text-sm font-semibold text-secondary leading-tight">{walkInCreating ? 'Adding\u2026' : 'Walk-in customer'}</p>
                      <p className="text-[11px] text-tertiary">Quick select — no registration needed</p>
                    </div>
                  ...
                  {selectedCustomer && !selectedCustomer.name.toLowerCase().includes('walk') && (
                    <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-subtle rounded-lg">
                      ...
                      <div><p className="text-xs font-semibold text-primary">{selectedCustomer.name}</p><p className="text-[11px] text-tertiary">{selectedCustomer.phone}</p></div>
                    </div>
                  )}
                </div>
                ...
                {catalog.length > 0 && (
                  <div className="px-5 py-4 border-b border-theme">
                    <div className="flex items-center gap-2 mb-3"><ShoppingCart className="w-3.5 h-3.5 text-tertiary" /><p className="text-[11px] font-semibold text-tertiary uppercase tracking-wider">Tap to add services</p></div>
                    <div className="space-y-3">
                      {Object.entries(catalogByCategory).map(([cat, svcs]) => (
                        <div key={cat}>
                          <p className="text-[10px] font-bold text-tertiary uppercase tracking-wider mb-2">{cat.replace('_', ' ')}</p>
                ...
                <div className="px-5 py-4 border-b border-theme">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[11px] font-semibold text-tertiary uppercase tracking-wider">Order items</p>
                  ...
                  {hasItems ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-12 gap-2 px-1">
                        <span className="col-span-6 text-[10px] font-semibold text-tertiary uppercase">Service</span>
                        <span className="col-span-2 text-[10px] font-semibold text-tertiary uppercase">Qty</span>
                        <span className="col-span-3 text-[10px] font-semibold text-tertiary uppercase">KES</span>
                      </div>
                      {itemRows.map((row, idx) => (
                        <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                          ...
                          <button type="button" onClick={() => removeItem(idx)} className="col-span-1 flex items-center justify-center text-disabled hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div>
                      <label className="block text-xs font-medium text-secondary mb-1.5">Manual total (KES) <span className="font-normal text-tertiary">— or tap catalog items above</span></label>
                ...
                {/* Dates & Notes */}
                <div className="px-5 py-4 border-b border-theme">
                  <p className="text-[11px] font-semibold text-tertiary uppercase tracking-wider mb-3">Dates &amp; Notes</p>
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                      <label className="block text-xs font-medium text-secondary mb-1.5"><Calendar className="inline w-3 h-3 mr-1 text-tertiary" />Pickup</label>
                      <DatePicker value={pickupDate} onChange={setPickupDate} placeholder="Pickup date" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-secondary mb-1.5"><Calendar className="inline w-3 h-3 mr-1 text-sky-400" />Delivery</label>
            ...
            <div className="px-5 py-4 border-t shrink-0" style={{ borderColor: "var(--border-default)", background: "var(--bg-surface)" }}>
              {hasItems && (
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs text-secondary">{itemRows.length} item{itemRows.length !== 1 ? 's' : ''}</span>
                  <span className="text-lg font-bold text-primary">KES {computeItemsTotal(itemRows).toLocaleString('en-KE', { minimumFractionDigits: 2 })}</span>
                </div>
              )}
        ...
        {STATUS_FLOW.map((s) => {
          ...
          return (
            <button key={s} onClick={() => setFilterStatus((p) => (p === s ? null : s))}
              className={`card px-4 py-3 text-left transition-all ${filterStatus === s ? 'ring-2 ring-orange-400 border-orange-300' : 'border-stone-200'}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {STATUS_FLOW.map((s) => (
            <button key={s} onClick={() => setFilterStatus((p) => (p === s ? null : s))}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${filterStatus === s ? STATUS_TAB_ACTIVE[s] : 'bg-surface border border-theme text-secondary hover:bg-subtle'}`}>
      ...
      {/* Orders list */}
      <div className="card">
        <div className="px-5 py-3 border-b border-theme flex items-center justify-between rounded-t-xl overflow-hidden">
          <p className="text-sm font-semibold text-primary">{filterStatus ? STATUS_LABEL[filterStatus] : 'All'} orders</p>
          <span className="text-xs text-tertiary">{visibleOrders.length} shown</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Package className="w-8 h-8 text-disabled mx-auto mb-2" />
            <p className="text-sm text-tertiary">{search ? 'No orders match your search' : filterStatus ? `No ${STATUS_LABEL[filterStatus].toLowerCase()} orders` : 'No orders yet'}</p>
          ...
          <div className="divide-y divide-[var(--border-default)]">
            {visibleOrders.map((order) => {
              ...
              return (
                <div key={order.id} className="px-5 py-4 hover:bg-subtle transition-colors">
                  {isEditing ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm font-medium text-primary">{customer?.name ?? 'Unknown'}</p>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div><label className="block text-xs font-medium text-secondary mb-1">Pickup date</label><DatePicker value={editPickupDate} onChange={setEditPickupDate} placeholder="Pickup date" /></div>
                        <div><label className="block text-xs font-medium text-secondary mb-1">Delivery date</label><DatePicker value={editDeliveryDate} onChange={setEditDeliveryDate} placeholder="Delivery date" /></div>
                      </div>
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="text-xs font-medium text-secondary">Items</label>
                          ...
                          <div className="space-y-2">
                            {editItems.map((row, idx) => (
                              <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                                ...
                                <button type="button" onClick={() => removeEditItem(idx)} className="col-span-1 flex items-center justify-center text-disabled hover:text-red-400"><Trash2 className="w-4 h-4" /></button>
                              </div>
                            ))}
                            <div className="flex justify-end pt-1 border-t border-theme">
                              <span className="text-xs text-secondary mr-2">Total:</span>
                              <span className="text-sm font-bold text-primary">KES {computeItemsTotal(editItems).toLocaleString('en-KE', { minimumFractionDigits: 2 })}</span>
                            </div>
                          </div>
                        ) : (
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={cancelEdit} className="text-xs text-secondary px-3 py-1.5 rounded-lg hover:bg-stone-100">Cancel</button>
                      </div>
                    </div>
                  ) : confirmDeleteId === order.id ? (
                    <div className="flex items-center gap-3 flex-wrap">
                      <p className="text-sm text-secondary">Delete this order?</p>
                      ...
                      <button onClick={() => setConfirmDeleteId(null)} className="text-xs text-secondary px-3 py-1.5 rounded-lg hover:bg-stone-100">Cancel</button>
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-sm font-semibold text-primary">{customer?.name ?? 'Unknown'}</p>
                            <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${badge}`}>{statusLabel}</span>
                          ...
                          {customer?.phone && <p className="text-xs text-tertiary mt-0.5">{customer.phone}</p>}
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-sm font-bold text-primary tabular-nums">KES {Number(order.total_amount).toLocaleString('en-KE', { minimumFractionDigits: 2 })}</p>
                          <p className="text-[11px] text-tertiary mt-0.5 flex items-center gap-1 justify-end"><Clock className="w-3 h-3" />{formatDate(order.created_at)}</p>
                        </div>
                      </div>
                      ...
                      {order.items?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-2">
                          {order.items.map((item, n) => (
                            <span key={n} className="inline-flex items-center gap-1 bg-stone-100 text-secondary rounded-lg px-2 py-0.5 text-[11px] font-medium">
                              {item.service_name} <span className="text-tertiary">×{item.quantity}</span>
                            </span>
                          ))}
                        </div>
                      )}
                      ...
                      {(order.notes || order.pickup_date || order.delivery_date || (order.worker_id && workerMap[order.worker_id])) && (
                        <div className="flex items-center gap-3 flex-wrap mb-2">
                          {order.pickup_date && <span className="flex items-center gap-1 text-[11px] text-secondary bg-subtle border border-stone-200 rounded-lg px-2 py-0.5"><Calendar className="w-3 h-3 text-tertiary" />Pickup {order.pickup_date}</span>}
                          ...
                          {order.notes && <span className="text-[11px] text-tertiary italic truncate max-w-[200px]">{order.notes}</span>}
                        </div>
                      )}

                      <div className="flex items-center gap-1 flex-wrap pt-1 border-t border-theme">
                        <button onClick={() => setReceiptOrder(order)} className="p-1.5 text-tertiary hover:text-secondary hover:bg-stone-100 rounded-lg transition-colors" title="Print receipt"><Printer className="w-3.5 h-3.5" /></button>
                        {isOwnerOrAdmin && (
                          <>
                            <button onClick={() => startEdit(order)} className="p-1.5 text-tertiary hover:text-secondary hover:bg-stone-100 rounded-lg transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                            <button onClick={() => setConfirmDeleteId(order.id)} className="p-1.5 text-tertiary hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                            {team.length > 0 && (
                              <div className="relative" ref={assigningOrderId === order.id ? assignRef : undefined}>
                                <button onClick={() => setAssigningOrderId((p) => (p === order.id ? null : order.id))}
                                  className={`p-1.5 rounded-lg transition-colors ${order.worker_id ? 'text-violet-600 bg-violet-50 hover:bg-violet-100' : 'text-tertiary hover:text-violet-600 hover:bg-violet-50'}`} title="Assign worker">
                                  <UserCircle2 className="w-3.5 h-3.5" />
                                </button>
                                {assigningOrderId === order.id && (
                                  <div className="absolute left-0 top-full mt-1 w-44 card rounded-xl shadow-lg z-20 overflow-hidden">
                                    <div className="px-3 py-2 border-b border-theme"><p className="text-[10px] font-semibold text-tertiary uppercase tracking-wide">Assign to</p></div>
                                    ...
                                    {team.filter((m) => m.role !== 'owner').map((m) => (
                                      <button key={m.user_id} onClick={() => assignWorker(order.id, m.user_id)}
                                        className={`w-full text-left px-3 py-2 text-xs transition-colors ${order.worker_id === m.user_id ? 'bg-violet-50 text-violet-700 font-semibold' : 'text-secondary hover:bg-subtle'}`}>
                                        {m.name} <span className="text-tertiary font-normal">({m.role})</span>
                                      </button>
                                    ))}
                                  </div>
                                )}
