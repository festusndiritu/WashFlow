...
const ROLE_BADGE: Record<string, string> = {
  ...
  worker: 'bg-stone-100 text-stone-600 border border-stone-200',
}
...
export function TeamPage() {
  ...
  const headerSlot = (
    <div className="flex items-center justify-between gap-4 flex-wrap">
      <div>
        <h1 className="text-base font-semibold text-stone-900 leading-none mb-0.5">Team</h1>
      ...
      <Modal open={showForm} onClose={() => { setShowForm(false); setFormError(null) }} title="Add team member" subtitle="Create login credentials for a new staff member">
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            {shops.length > 0 && (
              <div>
                <label className="block text-xs font-medium text-stone-600 mb-1">Assign to shop</label>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-stone-50">
            {members.map((m) => {
              ...
              if (isConfirm) {
                return (
                  <div key={m.user_id} className="px-5 py-4 flex items-center gap-3 flex-wrap bg-red-50">
                    <p className="text-sm text-stone-700">Remove <strong>{m.email}</strong> from the team?</p>
              ...
              return (
                <div key={m.user_id} className="px-5 py-3.5 flex items-center justify-between gap-4 hover:bg-stone-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-stone-100 flex items-center justify-center shrink-0">
                      <RoleIcon className="w-4 h-4 text-stone-500" />
                    </div>
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {m.shop_assignments?.length > 0 && (
                          <span className="text-xs text-stone-400">{m.shop_assignments.map((s) => s.shop_name).join(', ')}</span>
                        )}
                        {m.created_at && <span className="text-xs text-stone-300">since {formatDate(m.created_at)}</span>}
                      </div>
                    </div>
                  </div>
                  {canRemove && (
                    <button onClick={() => setConfirmDeleteId(m.user_id)}
                      className="p-1.5 text-stone-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors shrink-0" title="Remove member">
